namespace :sync do
  ...
  desc "Encolar job de sincronización completa de nuevos torneos (incluye placements)"
  task new_tournaments_complete: :environment do
    puts "🆕🏆 Encolando job de sincronización COMPLETA de nuevos torneos (incluye placements)..."
    puts "⚠️  Esto puede tomar más tiempo pero incluye todos los datos"

    job = SyncNewTournamentsJob.perform_later
    puts "✅ Job encolado con ID: #{job.job_id}"
    puts "🔍 Puedes monitorear el progreso en Mission Control: /jobs"
    puts ""
    puts "📝 NOTA: Los placements se sincronizan automáticamente después de los eventos"
  end

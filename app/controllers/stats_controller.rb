...
class StatsController < ApplicationController
  def index
    ...
    # Estadísticas de personajes más usados
  ...
  def calculate_character_stats
    # Personajes más usados (considerando character_1, character_2, character_3)
    all_characters = []

    # Obtener todos los personajes de todos los jugadores
    Player.where.not(character_1: [ nil, "" ]).pluck(:character_1).each { |char| all_characters << char }
    Player.where.not(character_2: [ nil, "" ]).pluck(:character_2).each { |char| all_characters << char }
    Player.where.not(character_3: [ nil, "" ]).pluck(:character_3).each { |char| all_characters << char }

    # Contar frecuencias
    character_counts = all_characters.group_by(&:itself).transform_values(&:size)

    # Ordenar y mostrar todos los personajes usados
    @most_used_characters_overall = character_counts
                                   .sort_by { |char, count| -count }
                                   .map { |char, count|
                                     {
                                       character: char,
                                       count: count,
                                       name: Player::SMASH_CHARACTERS[char] || char.humanize
                                     }
                                   }

    # Personajes principales más usados (solo character_1)
    main_character_counts = Player.where.not(character_1: [ nil, "" ])
                                 .group(:character_1)
                                 .count

    @most_used_main_characters = main_character_counts
                                .sort_by { |char, count| -count }
                                .map { |char, count|
                                  {
                                    character: char,
